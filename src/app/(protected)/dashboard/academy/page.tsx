...
interface Lesson {
  id: string;
  title: string;
  category: "strategy" | "defense" | "rhetoric" | "body" | "psychology" | "social-dynamics" | "negotiation" | "emotional-resilience" | "dark-influence" | "perception-management";
...
const LESSONS: Lesson[] = [
  ...
  },
  {
    ...
    howToApply: [
      ...
      "Alternatif Geliştirme (BATNA): Anlaşma olmaması durumunda başvuracağınız en iyi alternatifinizi (Best Alternative to a Negotiated Agreement) güçlü tutun.",
...
export default function AcademyPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<"all" | "strategy" | "defense" | "rhetoric" | "body" | "psychology" | "social-dynamics" | "negotiation" | "emotional-resilience" | "dark-influence" | "perception-management">("all");
  ...
  // Load specific lesson from URL parameter on mount
  useEffect(() => {
  ...
  const [videos, setVideos] = useState<VideoAnalysis[]>([
    {
      ...
      stressIndex: 35,
      credibilityIndex: 10,
      thumbnail: "/tyler_durden_cover.png",
...
Tyler, hipnotik bir sözel retorik kullanır. 'Tüketim toplumu karşıtı' argümanları rasyonel birer doktrin gibi sunarak zayıf ve anlam arayışındaki genç zihinleri manipüle eder. Onlara yapay bir aidiyet ve 'özgürlük' illüzyonu vererek kendi kurduğu klan yapısının (Project Mayhem) itaatkar piyonları haline getirir. Çerçeve kontrolü (frame control) o kadar güçlüdür ki, onunla etkileşime girenler kendi ahlaki ve rasyonel muhakemelerini askıya alarak onun gerçeklik alanına çekilirler.`,
    ...
    },
    {
      ...
      stressIndex: 15,
      credibilityIndex: 95,
      thumbnail: "/sherlock_holmes_cover.png",
...
Sherlock Holmes, duygusal veri akışını bilişsel düzeyde tamamen filtreleyerek rasyonel analize ve saf gözleme odaklanan soğuk bir zihinsel mimaridir. Klinik açıdan profili, aşırı gelişmiş bir analitik narsisizm and detay takıntısı içerir. Sherlock, sosyal yakınlığı ve empatiyi 'çalışma mekanizmasını yavaşlatan, rasyonel kararları gölgeleyen duygusal bir kirlilik' olarak tanımlayarak bilinçli bir 'Rasyonel Mesafe' (rational distance) protokolü geliştirmiştir. Karşısındaki insanları insani özneler olarak değil, çözülmesi ve deşifre edilmesi gereken birer veri yığını ve bulmaca olarak görür.
...
Nezaket kurallarını ve sosyal protokolleri bilinçli olarak reddetmesi, yapısal bir iletişim bozukluğundan ziyade, kendi zihinsel üstünlüğünü ve bağımsız çerçevesini koruma amaçlı bir güç strategisidir. Kendini sosyal normların üzerinde konumlandırarak, etkileşime girdiği herkesi kendi kurallarının geçerli olduğu rasyonel bir alana çekmeye zorlar.`,
    ...
    },
    {
      ...
      stressIndex: 60,
      credibilityIndex: 25,
      thumbnail: "/walter_white_cover.png",
...
Walter, çevresindekileri yönetmek için suçluluk duygusunu kullanır ve kurban rolünü (mağduriyet maskesi) mükemmel şekilde oynar. Kendi ahlaki çöküşünü haklı çıkarmak için bilişsel çelişki (cognitive dissonance) yöntemine sığınır. Masada köşeye sıkıştığında savunma yapmak yerine karşı saldırı protokolüne (counter-attack) geçerek muhataplarının zayıf noktalarına ve korkularına (örn: Jesse Pinkman'in onaylanma ihtiyacı) oynar. Güç sahibi oldukça empati yeteneğini tamamen yitirerek, kendi kurduğu uyuşturucu imparatorluğunda mutlak ve acımasız bir otorite haline gelir.`,
    ...
    },
    {
      ...
      stressIndex: 85,
      credibilityIndex: 30,
      thumbnail: "/anakin_skywalker_cover.png",
...
Duygu durumundaki aşırı dalgalanmalar, kimlik karmaşası (Jedi Şövalyeliğinden Sith Lordluğuna geçiş) ve impulsif (dürtüsel) eylemleri Borderline yapısının çekirdeğini oluşturur. Padmé'yi kaybetme korkusu, onun rasyonel düşünme yetisini felç ederek bilişsel çelişkiyi zirveye taşımış ve Palpatine'in manipülasyonlarına karşı onu tamamen savunmasız bırakmıştır. Darth Vader maskesini taktıktan sonra beden dilinde mutlak bir fiziksel kapatma (duygusal poker face) ve heybetli, yavaş, tehditkar hareketlerle mekansal dominans kurulmuştur. Amigdalası artık stres hissetmek yerine saf öfke ve yıkım üretmektedir.`,
    ...
    },
    {
      ...
      stressIndex: 5,
      credibilityIndex: 15,
      thumbnail: "/hannibal_lecter_cover.png",
...
Beden dili hipnotik düzeyde sabittir. Göz kırpma refleksi neredeyse yok denecek kadar azdır; bu da muhatabında av-avcı ilişkisindeki av konumuna düşme hissi uyandırır. Duruşu asil, sakin ve hareketleri son derece kontrollüdür. Karşı tarafı manipüle etmek için onların bastırılmış travmalarını ve bilinçaltı zafiyetlerini bir cerrah titizliğiyle analiz eder ve konuşurken yavaş, melodik ve ritmik bir ses tonu kullanarak bilişsel savunma hatlarını (ego depletion) felç eder.`,
    ...
    },
    {
      ...
      stressIndex: 40,
      credibilityIndex: 70,
      thumbnail: "/patrick_jane_youtube_kapak.png",
      videoUrl: "/patrick_jane_youtube.mp4",
...
Davranışsal olarak, sürekli sergilediği sıcak, sempatik, hatta çocuksu maske (hale etkisi) muhataplarının gardını tamamen düşürmek için tasarlanmıştır. Çerçeve kontrolü (frame control) son derece esnektir; doğrudan çatışmak yerine karşı tarafın shit-test'lerini mizahla ve absürtlükle bozar. Gözleri sürekli aktiftir; ortamdaki ve kişilerdeki mikro ipuçlarını toplar. Elleriyle yaptığı rahatlatıcı ve yönlendirici jestler, muhataplarının bilinçaltına telkinler göndermesini sağlar.`,
      ...
      videoSources: [
        {
          title: "Patrick Jane (The Mentalist) - YouTube Analizi",
          thumbnail: "/patrick_jane_youtube_kapak.png",
          videoUrl: "/patrick_jane_youtube.mp4"
        },
        {
          title: "Patrick Jane (The Mentalist) - Mikro İfade ve İllüzyonist Profilleme",
          thumbnail: "/patrick_jane_cover.png",
    ...
    },
    {
      ...
      stressIndex: 50,
      credibilityIndex: 20,
      thumbnail: "/joe_goldberg_cover.png",
...
Beden dili dışarıdan bakıldığında son derece uysal, içe dönük ve zararsız bir entelektüel izlenimi verir (Hale Etkisi). Omuzları hafifçe çökük, ses tonu yumuşak ve konuşması ölçülüdür. Ancak iç dünyasında sürekli akan paranoid iç konuşmalar, hedefiyle göz teması kurduğu anlarda göz bebeklerinin büyümesi ve mikro yutkunmalarla dışa vurulan otonom stres kaçaklarına yol açar. Love Bombing (aşk bombardımanı) evresinde hedefine aşırı korumacı ve ilgi dolu yaklaşarak onun duygusal bağımsızlığını yok eder ve kendine bağımlı kılar.`,
    ...
    },
    {
      ...
      stressIndex: 20,
      credibilityIndex: 85,
      thumbnail: "/thomas_shelby_cover.png",
...
Beden dili mutlak bir hareketsizlik ve kontrol üzerine kuruludur. En büyük kriz anlarında bile yüz kasları oynamaz (mükemmel poker face). Gözleri buz gibi soğuk, delici ve odaklıdır. Konuşurken sergilediği yavaşlık ve kurduğu uzun, stratejik duraklamalar (sessizlik ilkesi), masadaki muhatapları üzerinde muazzam bir psikolojik baskı yaratır ve onları panik halinde hata yapmaya zorlar. Sigarasını yakarken veya içkisini yudumlarken yaptığı yavaş ve kontrollü hareketler, zamanı ve ortamı kendi kontrolünde tuttuğunun sözsüz birer ilanıdır.`,
      advice: "Thomas Shelby profilleriyle pazarlık masasında çatışmaya girmeyin. Onun sessizlik silahına karşı sessizlikle yanıt verin ve reaktifliğinizi sıfırlayın. Masadaki tek geçerli kozunuz, onun rasyonel iş hedeflerine sunacağınız somut, ölçülebilir faydalar ve masadan kalkıp gidebilme gücünüzdür."
    },
    {
      id: "dexter_morgan",
      ...
      stressIndex: 10,
      credibilityIndex: 40,
      thumbnail: "/dexter_morgan_cover.png",
...
Normal insan taklidi yaparken beden dili bilinçli olarak 'fazla' sosyal ve cana yakındır; sık sık yapay gülümsemeler sergiler (Duchenne olmayan, göz kaslarının dahil olmadığı sahte gülüşler). Kan lekelerini analiz ederken veya avının peşindeyken ise yüzü tamamen soğur, kasları gevşer ve bakışları keskinleşir (avcı modu). Otonom sinir sistemi uyarılmalarını bilinçli olarak baskılamayı öğrenmiştir; bu yüzden yalan dedektörlerini bile kolaylıkla atlatabilir.`,
    ...
    },
    {
      ...
      stressIndex: 30,
      credibilityIndex: 90,
      thumbnail: "/michael_scofield_cover.png",
...
Light, başlangıçta kusursuz bir örnek öğrenci maskesi taşır. Beden dili son derece nazik, kibar ve simetriktir. Ancak köşeye sıkıştığında ve L veya Near tarafından zihinsel çerçevesi zorlandığında, otonom sinir sistemi çökmeye başlar; göz bebekleri çılgınca büyür, ses tonu tizleşir ve histerik kahkahalar atar (çerçeve parçalanması). Yalan söylerken gözlerini hafifçe kısarak rasyonel kurgu moduna geçtiğini ifşa eden mikro ifadeler sergiler.`,
    ...
    },
    {
      ...
      stressIndex: 25,
      credibilityIndex: 80,
      thumbnail: "/harvey_specter_cover.png",
...
Beden dili alfa statü sinyalleriyle doludur. Kusursuz takım elbiseleri, dik omuzları ve çene hattının yukarıda olması (postür dominansı) masaya oturduğu an üstünlüğü kurmasını sağlar. Göz teması delici, kararlı ve meydan okuyucudur. Müzakerelerde algı çapalama (anchoring) ve blöf yapma tekniklerini ustalıkla kullanır. Stres altındayken bile göz kırpma hızı değişmez, ses tonu sabit kalır; bu da onun duygusal çelişkilerini dışarıya sızdırmasını engeller.`,
    ...
    },
    {
      ...
      stressIndex: 10,
      credibilityIndex: 20,
      thumbnail: "/frank_underwood_cover.png",
...
Frank, rakiplerini zayıflatmak için bilgi asimetrisini (information asymmetry) ve çifte bağ (double bind) yöntemlerini ustalıkla kullanır. Karşı tarafın hırslarını ve zaaflarını tespit edip buralara kademeli baskı (pressure points) uygular. Masada asla reaktif davranmaz; kışkırtmalara stoik bir sakinlikle karşılık vererek rakibinin bilişsel enerjisini tüketir (ego depletion).`,
    ...
    },
    {
      ...
      stressIndex: 75,
      credibilityIndex: 15,
      thumbnail: "/homelander_cover.png",
...
Homelander, çevresindekileri yönetmek için saf fiziksel terör ve gaslighting yöntemlerini kullanır. Muhataplarını sürekli çifte bağ (double bind) durumunda bırakarak (örn: 'Beni seviyor musun yoksa benden korkuyor musun?') onları zihinsel felce uğratır.`,
    ...
    },
    {
      ...
      stressIndex: 45,
      credibilityIndex: 80,
      thumbnail: "/aemond_targaryen_cover.png",
...
Müzakerelerde ve sosyal etkileşimlerde sessizlik ilkesini (silence) ve alan kaplamayı (spatial dominance) kullanarak muhataplarını küçümser. Kendi çerçevesini sarsılmaz bir stoik zırh gibi taşır.`,
    ...
    },
    {
      ...
      stressIndex: 20,
      credibilityIndex: 90,
      thumbnail: "/rust_cohle_cover.png",
...
Rust, klasik narsisistik manipülasyonlara (Love Bombing, Gaslighting vb.) karşı tamamen bağışıktır, çünkü dünyevi hiçbir beklentisi, ego arzusu veya onaylanma ihtiyacı kalmamıştır.`,
    ...
    },
    {
      ...
      stressIndex: 40,
      credibilityIndex: 15,
      thumbnail: "/jordan_belfort_cover.png",
...
İnsanların 'zengin olma' ve 'statü kazanma' şemalarını tetikleyerek onları kendi kurduğu finansal manipülasyon ağının içine çeker.`,
    ...
    },
    {
      ...
      stressIndex: 30,
      credibilityIndex: 10,
      thumbnail: "/patrick_bateman_cover.png",
...
Bateman için sosyal statü ve onaylanma hayatta kalma meselesidir. Kendisini başkalarıyla kıyaslamadan var olamaz.`,
    ...
    {
      id: "don_draper",
      title: "Don Draper (Mad Men) - Karizma, Kaçınma ve Algı Yönetimi",
      description: "Narsisistik kaçınma şeması, hipnotik duruş ve profesyonel algı manipülasyonu.",
      url: "Dizi: Mad Men (AMC)",
      status: "completed",
      personality: "Kaçınan/Narsisistik Kompanzasyon & Karizmatik Uyum",
      stressIndex: 30,
      credibilityIndex: 65,
      thumbnail: "/don_draper_cover.png",
      videoUrl: "/Don Draper.mp4",
      detailedAnalysis: `KLİNİK TEŞHİS: Kaçınan Kişilik Eğilimleri, Narsisistik Aşırı Telafi ve Yoğun Kimlik Karmaşası (Dick Whitman / Don Draper bölünmesi).

Don Draper, geçmişindeki ağır çocukluk travmalarını, yoksulluğu ve değersizlik şemalarını tamamen yok ederek 'kusursuz karizmatik lider' kimliğiyle aşırı kompanse (narcissistic overcompensation) etmiş bir figürdür. Klinik açıdan Don, yakın ilişkilerden kaçınır (avoidant attachment); çünkü duygusal yakınlık onun sahte kimliğinin deşifre olma riskini (değersizlik korkusu) taşır. Alkol ve cinsel bağımlılıkları, içsel boşluğunu ve travmatik anılarını bastırma amacı taşıyan otonom regülasyon çabalarıdır.

BEDEN DİLİ VE STATÜ SİNYALLERİ:
Don'ın duruşu mutlak bir 'zarafet ve güç' dengesidir. Omuzları dik, hareketleri son derece yavaş ve telaşsızdır. Sigarasını tutuşu, bardağını kavrayışı ve oturuş biçimi mekansal dominansını (territorial dominance) pekiştirir. Göz teması uzun, derin ve güven vericidir; ancak bu bakışların arkasında derin bir duygusal soğukluk ve gözlemci mesafe yatar. Müzakerelerde ses tonunu asla yükseltmez; alçak, tok ve pürüzsüz bir frekans kullanarak hipnotik bir çerçeve kontrolü kurar.

ALGILARIN YÖNETİMİ VE RETORİK:
Don Draper, insanların arzularını, korkularını ve eksikliklerini anlama konusunda olağanüstü bir sezgiye sahiptir. Reklam sunumlarında (örn: Kodak Carousel) rasyonel argümanlar yerine duygusal çapa (emotional anchoring) ve nostalji şemalarını tetikleyerek muhataplarının savunma kalkanlarını tamamen eritir. Çerçeve çatışmalarında doğrudan savunma yapmak yerine, konuyu tamamen değiştirerek kendi kurallarının geçerli olduğu yeni bir algı alanı yaratır.`,
      advice: "Don Draper tarzı karizmatik ve kaçınan narsistlerle çalışırken onların profesyonel dehasını kabul edin ancak duygusal alanlarına girmekten kaçının. Müzakerelerde rasyonel çıkar dengelerini koruyun, onun sunduğu duygusal çapalara kapılmadan katı verilere odaklanın."
    },
    {
      id: "danearys",
      title: "Daenerys Targaryen (Game of Thrones) - Mesih Kompleksi ve Karizmatik Otorite",
      description: "Kurban rolünden kurtarıcı ve mutlak hak sahibi ejderha kimliğine geçişin klinik analizi.",
      url: "Dizi: Game of Thrones (HBO)",
      status: "completed",
      personality: "Kurtarıcı/Mesih Kompleksi (Megalomani) & Karizmatik Liderlik",
      stressIndex: 55,
      credibilityIndex: 75,
      thumbnail: "/danearys_cover.png",
      videoUrl: "/danearys.mp4",
      detailedAnalysis: `KLİNİK TEŞHİS: Mesih Kompleksi (Messiah Complex), Narsisistik Megalomani ve Ağır Güç/Hak Sahibi Olma (Entitlement) Şemaları.

Daenerys Targaryen, sürgün, zulüm ve cinsel/fiziksel istismar ile geçen erken dönem yaşamının ardından, kendisini 'Ezilenlerin Kurtarıcısı' ve 'Mutlak Hak Sahibi Kraliçe' (the rightful ruler) olarak konumlandırarak derin travmalarını aşırı kompanse etmiştir. Klinik açıdan Daenerys, zamanla 'kendi adaletini mutlak ahlak' olarak gören sanrılı bir bilişsel çerçeveye kaymıştır. İnancı o kadar katıdır ki, onun hedeflerine karşı çıkan herkes otomatik olarak 'kötü' veya 'tiran' olarak etiketlenir ve yok edilmesi meşrulaştırılır (rasyonalizasyon).

BEDEN DİLİ VE OTORİTER DOMİNANS:
Daenerys'in beden dili, zamanla kırılgan bir sürgünden mutlak bir otoriteye evrilmiştir. Çene hattı dik, bakışları sabit ve göz kırpma refleksleri son derece düşüktür. Elleri genellikle önünde birleşik veya taht kollarında sabitlenmiştir; bu da mekansal kontrolünü gösterir. Ejderhalarının varlığı, onun kişisel güç alanını genişleterek beden diline stoik ve sarsılmaz bir özgüven olarak yansır. Stres altındayken omuzları gerilir ve ses tonu aniden tizleşerek buyurgan bir tona bürünür.

RETORİK VE İNANDIRICILIK:
Konuşmalarında 'Zincirleri Kıran' (Breaker of Chains) ve 'Ejderhaların Anası' (Mother of Dragons) gibi unvanlarını sürekli tekrarlayarak kendine ve kitlelere karizmatik bir mitolojik kimlik dikte eder. İnanç temelli manipülasyonu o kadar güçlüdür ki, takipçileri kendi rasyonel çıkarlarını unutup onun mesihsel vizyonuna biat ederler.`,
      advice: "Daenerys tarzı inanç sanrılarına ve kurtarıcı kompleksine sahip figürlerle iletişim kurarken onun ideallerini ve misyonunu asla doğrudan sorgulamayın (die narsisistik öfkeyi tetikler). Onun vizyonunu onaylıyormuş gibi görünerek, eylemlerinin kendi 'kurtarıcı' imajına ve hedeflerine nasıl zarar verebileceğini rasyonel olarak göstererek onu yönlendirin."
    }
  ]);
              ...
              {selectedLesson.id === "person-psychology" ? (
                ...
                <div className="space-y-6 overflow-x-hidden">
                  ...
                  {(() => {
                    ...
                    const hasMultipleSources = currentVideo.videoSources && currentVideo.videoSources.length > 1;
                    const currentSource = hasMultipleSources && currentVideo.videoSources
                      ? currentVideo.videoSources[activeVideoIndex]
                      : { videoUrl: currentVideo.videoUrl, thumbnail: currentVideo.thumbnail, title: currentVideo.title };
                        ...
                        >
                          ...
                          {currentVideo.status === "processing" ? (
                            ...
                            <>
                              ...
                              {/* Slider Controls */}
                              {hasMultipleSources && currentVideo.videoSources && (
                                <>
                                  {/* Left Arrow */}
                                  <button
                                    onClick={(e) => {
                                      ...
                                      setActiveVideoIndex((prev) => 
                                        prev === 0 ? currentVideo.videoSources!.length - 1 : prev - 1
                                      );
                                    }}
                                    className="absolute left-3 top-1/2 -translate-y-1/2 bg-void/90 hover:bg-gold/15 border border-gold/20 hover:border-gold/50 hover:text-gold rounded-full p-1.5 text-ash z-30 transition-all cursor-pointer shadow-lg backdrop-blur-sm"
                                    title="Önceki Video"
                                  >
                                    <ArrowLeft className="w-3.5 h-3.5" />
                                  </button>
                                  ...
                                  <button
                                    onClick={(e) => {
                                      ...
                                      setIsPlaying(false);
                                      setActiveVideoIndex((prev) => 
                                        prev === currentVideo.videoSources!.length - 1 ? 0 : prev + 1
                                      );
                                    }}
                                    className="absolute right-3 top-1/2 -translate-y-1/2 bg-void/90 hover:bg-gold/15 border border-gold/20 hover:border-gold/50 hover:text-gold rounded-full p-1.5 text-ash z-30 transition-all cursor-pointer shadow-lg backdrop-blur-sm"
                                    title="Sonraki Video"
                                  >
                                  ...
                                  {/* Dot Indicators */}
                                  <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-1.5 z-30 bg-void/60 backdrop-blur-sm px-2.5 py-1 rounded-full border border-obsidian/60">
                                    {currentVideo.videoSources.map((_, idx) => (
                                      <button
                                        key={idx}
                                        onClick={(e) => {
                                          e.stopPropagation();
                                        ...
                                        className={`w-1.5 h-1.5 rounded-full transition-all ${
                                          activeVideoIndex === idx ? "bg-gold scale-125" : "bg-ash/40 hover:bg-ash/70"
                                        }`}
                                      />
                                    ))}
                                  </div>
                              ...
                              {isPlaying && currentSource.videoUrl ? (
                                <video
                                  ref={videoRef}
                              ...
                              >
                                <span className="truncate max-w-[120px] md:max-w-[200px]">{currentSource.title}</span>
          ...
          <div className="w-full flex flex-col md:flex-row gap-4 justify-between items-center bg-abyss/45 border border-obsidian/60 p-4 rounded-sm">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {[
                ...
                { id: "social-dynamics", label: "Sosyal Dinamikler" },
                { id: "negotiation", label: "Müzakere & İkna" },
                { id: "emotional-resilience", label: "Duygusal Dayanıklılık" },
                { id: "dark-influence", label: "Karanlık Nüfuz" },
                { id: "perception-management", label: "Algı Yönetimi" }
              ].map((cat) => (
                <button
                  key={cat.id}
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      <span className={`inline-block text-[8px] font-accent tracking-widest uppercase px-2 py-0.5 border rounded-sm ${
                        ...
                        lesson.category === "social-dynamics" ? "text-indigo-400 bg-indigo-950/10 border-indigo-900/30" :
                        lesson.category === "negotiation" ? "text-cyan-400 bg-cyan-950/10 border-cyan-900/30" :
                        lesson.category === "emotional-resilience" ? "text-rose-400 bg-rose-950/10 border-rose-900/30" :
                        lesson.category === "dark-influence" ? "text-violet-400 bg-violet-950/10 border-violet-900/30" :
                        lesson.category === "perception-management" ? "text-orange-400 bg-orange-950/10 border-orange-900/30" :
